...
export const CasImportService = {
  ...
  async importStatement(
    ...
    /*
     * The browser hands us a DOM File; a phone hands us a path and a base64
     * blob. Both end in the same call, so this reads the File and delegates.
     */
    return CasImportService.importStatementBytes(
      { fileName: file.name, fileSize: file.size, base64: await toBase64(file) },
      password,
      requestId,
    );
  },
  ...
  /**
   * The platform-neutral form: bytes that are already base64, plus the name and
   * size the checks need. React Native has no `File`, and reading a picked
   * document there produces base64 directly.
   */
  async importStatementBytes(
    input: { fileName: string; fileSize: number; base64: string },
    password: string,
    requestId?: string | null,
  ): Promise<CasImportResponse> {
    const file = input;
    if (file.fileSize > MAX_CAS_BYTES) {
