// Partner places an unlisted-share order for one of their clients. Pick a client,
// set the per-share margin (0–5%, defaults to the partner's global spread), choose
// a quantity, review the indicative amount at the partner price, and submit. The
...
export function PartnerShareOrderModal({
  share,
  defaultMargin,
  onClose,
  onPlaced,
  ...
  const place = async () => {
    ...
    setError(null);
    try {
      const order = await PartnerService.placeShareOrder({ clientId, shareId: share.id, qty, margin: marginNum });
        ...
        {placedRef ? (
          <div className="p-6 text-center">
            ...
            <p className="mx-auto mt-2 max-w-xs text-sm text-text-secondary">
              Order <span className="font-semibold text-text-primary">{placedRef}</span> for {qty} share
              {qty === 1 ? '' : 's'} has been sent to the client's relationship manager, who will confirm availability
              and the deal.
            </p>
            <button type="button" onClick={onPlaced}
          ...
          <div className="space-y-4 overflow-y-auto p-5">
            <p className="text-sm font-bold text-text-primary">{share.short_name || share.company_name}</p>
