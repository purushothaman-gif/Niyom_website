// Partner shares an unlisted share with a client via a link carrying a per-share
// margin. The recipient opens /share-offer?t=<token> to see the share at the
// partner's price and request to buy (routed to the RM). The partner's cost is
// never in the link or on the page — the server computes one price and returns
// only that.
...
import {
  clampShareMargin, isShareMarginValid, partnerSharePrice, MAX_PARTNER_SHARE_MARGIN,
} from '../../../../shared/partner/shares/partnerShareMath';
import { PartnerService, type PartnerShare } from '../../services/PartnerService';

export function PartnerShareLinkModal({
  share,
  defaultMargin,
  onClose,
}: {
  ...
  const preview = partnerSharePrice(share, margin);
  const marginValid = isShareMarginValid(margin);
  const name = share.short_name || share.company_name;

  const generate = async () => {
    setBusy(true);
    setError(null);
    try {
      const token = await PartnerService.createShareLink(share.id, marginNum);
      setLink(`${window.location.origin}/share-offer?t=${token}`);
    } catch (e) {
  ...
  const shareText = link ? `${name} (unlisted) — view details & invest: ${link}` : '';
      ...
      <div className="w-full max-w-sm rounded-token-xl border border-border bg-bg-elevated shadow-token-lg" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-4 p-5">
          <p className="text-sm text-text-secondary">
            A link for <strong className="text-text-primary">{name}</strong>. Your client sees the share at your price
            and can request to buy — the order comes to your RM.
          </p>
            ...
            <>
              <label className="block">
                ...
                <div className="relative">
                  ...
                  <input type="number" step="0.01" min={0} max={MAX_PARTNER_SHARE_MARGIN} value={margin}
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-3">
                <button type="button" onClick={copy}
                  className="inline-flex items-center justify-center gap-1.5 rounded-token-md border border-border bg-bg-surface py-2.5 text-sm font-bold text-text-primary hover:border-accent/40">
