// Bond orders the partner has raised for their clients, newest first, with the
// RM's progress (Submitted → Deal sent → Accepted).

import { Landmark } from 'lucide-react';
import { inr, dateTime, pct } from '../../../lib/money';
import type { PartnerBondOrder, PartnerBondOrderStatus } from '../../services/PartnerService';
...
export function PartnerMyOrders({ orders, onExplore }: { orders: PartnerBondOrder[]; onExplore: () => void }) {
  if (orders.length === 0) {
    return (
      <div className="rounded-token-xl border border-border bg-bg-elevated py-16 text-center">
        ...
        <p className="mx-auto mt-1 max-w-sm text-sm text-text-muted">Orders you place for your clients appear here, so you can track the RM's confirmation.</p>
        <button type="button" onClick={onExplore} className="mt-4 rounded-token-md bg-accent px-4 py-2 text-sm font-bold text-on-accent">Explore bonds</button>
      </div>
    );
  }
          ...
          <div key={o.id} className="flex flex-wrap items-center justify-between gap-3 rounded-token-xl border border-border bg-bg-elevated p-5 shadow-token-card">
            <div className="min-w-0">
              ...
              <p className="mt-0.5 text-[11px] text-text-faint">
                {o.ref} · {o.client?.full_name ?? '—'} · {o.units} unit{o.units === 1 ? '' : 's'} @ {inr(o.price_per_100)}/₹100
