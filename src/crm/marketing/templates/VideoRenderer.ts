...
function drawScene(
  ...
  const alpha = fade(t);
  const rise = (1 - easeOutCubic(clamp01(t / 0.22))) * 44 * scale;
  ...
  const startY = (h - blockH) / 2 + fontSize * 0.34 + rise;
...
export async function renderVideoFrame(
  ...
  if (idx < req.scenes.length) {
    drawScene(ctx, width, height, palette, req.scenes[idx], idx, units,
      req.category, req.disclaimer, local);
  } else {
