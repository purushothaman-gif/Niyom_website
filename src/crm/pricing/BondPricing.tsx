...
import { Loader2, Percent, Check, X, ShieldAlert, Search, Users, Handshake, Landmark } from 'lucide-react';
import { NWEmployee } from '../types';
import {
  pricingQueryClient, useMarkups, useMyClients, useMyPartners, usePropose, useApprove, useReject,
  MarkupRow, NamedRow, Audience,
} from './pricingClient';
...
function Inner({ employee }: Props) {
  ...
  const { data: markups = [], isLoading } = useMarkups();
  ...
  return (
    <div className="space-y-5">
      <div>
        <p className="text-xs uppercase tracking-widest mb-1" style={{ color: 'var(--accent)' }}>Bond Pricing</p>
        <h1 className="text-2xl font-bold" style={{ color: 'var(--text-primary)' }}>Client &amp; Partner Markups</h1>
        <p className="text-sm mt-1" style={{ color: 'var(--text-faint)' }}>Set an added-revenue % on the base bond price. Rates go live only after admin approval.</p>
      </div>
      ...
      <div className="rounded-2xl overflow-hidden" style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)' }}>
        ...
        {isLoading ? (
          ...
          <p className="text-sm px-5 py-10 text-center" style={{ color: 'var(--text-faint)' }}><Landmark className="w-6 h-6 mx-auto mb-2" style={{ color: 'var(--text-faint)' }} />No {tab}s.</p>
        ) : (
...
export default function BondPricing({ employee }: Props) {
  return <QueryClientProvider client={pricingQueryClient}><Inner employee={employee} /></QueryClientProvider>;
}
