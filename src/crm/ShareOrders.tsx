// Unlisted Share Orders — the RM's inbox for orders placed from the client
// portal, the partner portal, or a partner's shared offer link.
//
// The bond twin of this screen is BondOrders.tsx. Kept as its own component
// rather than generalised: a share order has no face value and no price-per-₹100,
// the deal line it prepares is an Unlisted Share rather than a Secondary Bond,
// and merging the two would have meant a product branch in every cell.

import { useCallback, useEffect, useMemo, useState } from 'react';
import {
  Gem, Search, RefreshCw, X, Mail, Phone, User, Clock, Inbox, FileText, ExternalLink,
} from 'lucide-react';
...
function StatusPill({ status }: { status: OrderStatus }) {
  const m = STATUSES.find((x) => x.value === status) ?? STATUSES[0];
  return (
...
export default function ShareOrders({ employee, onNavigate }: Props) {
  ...
  const updateStatus = async (o: OrderRow, status: OrderStatus) => {
    setSaving(true);
    const { error: err } = await supabase.from('nw_share_orders').update({ status }).eq('id', o.id);
    setSaving(false);
  ...
  // "Prepare Deal Confirmation" — the share price IS the per-unit rate, so unlike
  // bonds there is no face-value conversion to do here.
  const prepareDeal = (o: OrderRow) => {
    ...
    onNavigate('deal_confirmation', {
      ...
      prefillType: 'Buy',
      prefillProduct: 'Unlisted Share',
      prefillSecurity: o.company_name,
      prefillIsin: o.isin,
      prefillQty: String(o.qty),
      prefillRate: String(o.price_per_share),
    });
  };
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <p className="text-xs uppercase tracking-widest mb-1" style={{ color: 'var(--accent)' }}>Alternate Investments</p>
          <h1 className="text-2xl font-bold text-text-primary flex items-center gap-2">
            <Gem className="w-6 h-6" style={{ color: 'var(--accent)' }} /> Share Orders
          </h1>
          <p className="text-xs mt-1" style={{ color: 'var(--text-muted)' }}>
            {isAdmin
              ? 'All unlisted share orders placed from the client and partner portals.'
              : 'Unlisted share orders placed by your clients and partners.'}
          </p>
        </div>
        <button onClick={load}
      ...
      <div className="flex flex-wrap gap-3">
        <div className="relative flex-1 min-w-48">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)}
            placeholder="Search ref, company, ISIN or client..."
      ...
      <div className="rounded-2xl overflow-hidden" style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border)' }}>
        {loading ? (
          <div className="flex items-center justify-center py-16"><LogoLoader size={48} /></div>
        ) : visible.length === 0 ? (
            ...
            <table className="w-full nw-table">
              <thead>
                <tr style={{ borderBottom: '1px solid var(--border-subtle)' }}>
                  {['Ref', 'Client', 'Company', 'Qty', 'Indicative', 'Status', 'Placed', ''].map((h) => (
              ...
              <tbody>
                {visible.map((o) => (
                  <tr key={o.id} onClick={() => setActive(o)} className="cursor-pointer transition-colors"
                    ...
                    <td className="px-5 py-3.5 text-sm text-text-primary max-w-xs truncate">{o.company_name || o.isin}</td>
                    <td className="px-5 py-3.5 text-sm" style={{ color: 'var(--text-secondary)' }}>{o.qty}</td>
                    ...
                    <td className="px-5 py-3.5 text-right">
                      <button onClick={(e) => { e.stopPropagation(); setActive(o); }}
                        className="text-xs font-semibold" style={{ color: 'var(--accent)' }}>View</button>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {active && (
        <div className="fixed inset-0 z-[70] flex justify-end" style={{ background: 'rgba(0,0,0,0.4)' }} onClick={() => setActive(null)}>
          <div className="h-full w-full max-w-md overflow-y-auto p-6"
            ...
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <h2 className="mt-1 text-lg font-bold text-text-primary">{active.company_name || active.isin}</h2>
            ...
            <div className="mt-5 rounded-xl p-4 space-y-2" style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border)' }}>
              ...
              {active.client?.email && (
                <a href={`mailto:${active.client.email}?subject=Re: ${encodeURIComponent(active.ref + ' — ' + active.company_name)}`}
            ...
            <div className="mt-4 rounded-xl p-4 space-y-2.5" style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border)' }}>
              ...
              <DrawerRow label="Quantity" value={`${active.qty} share${active.qty === 1 ? '' : 's'}`} />
              <DrawerRow label="Price / share" value={inr(active.price_per_share)} />
              ...
              {active.notes && <p className="pt-1 text-xs" style={{ color: 'var(--text-muted)' }}>Note: {active.notes}</p>}
            </div>

            <button onClick={() => prepareDeal(active)}
            ...
            {active.deal_id && onNavigate && (
              <button onClick={() => onNavigate('deal_confirmation')}
                className="mt-3 w-full flex items-center justify-center gap-1.5 text-xs font-semibold" style={{ color: 'var(--accent)' }}>
            ...
            <div className="mt-6">
              ...
              <div className="grid grid-cols-2 gap-2">
                {STATUSES.map((s) => {
                  ...
                  return (
                    <button key={s.value} disabled={saving} onClick={() => updateStatus(active, s.value)}
...
function DrawerRow({ label, value, strong, mono }: { label: string; value: string; strong?: boolean; mono?: boolean }) {
  return (
    <div className="flex items-center justify-between gap-3">
      ...
      <span className={`text-right ${mono ? 'font-mono text-xs' : strong ? 'text-sm font-bold' : 'text-xs font-semibold'} text-text-primary`}>
        {value}
      </span>
    </div>
  );
