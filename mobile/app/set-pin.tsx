/**
 * Setting (or replacing) the device PIN, and turning on biometric unlock.
 *
 * ## Why a PIN at all
 *
 * Five minutes of inactivity signs a client out, matching the website. That is
 ...
 *
 * The PIN goes to the SERVER (`client-pin-set`), hashed, against this device's
 * id. Nothing on the phone can verify it — the device only ever asks, and
 * `client-pin-login` decides, counts the failures, cools the device off after
 * five and burns the PIN after ten.
 *
 * A copy is kept locally ONLY if the client turns on Face ID, and only inside
...
import { clientSetPin } from '@/features/auth/authApi';
...
export default function SetPin() {
  ...
  const onConfirmComplete = async (pin: string) => {
    ...
    const { ok, data } = await clientSetPin(deviceId, pin, label);
    setBusy(false);
    ...
    await saveProfile('client', {
      id: clientId,
      name: client?.full_name ?? 'Your account',
      maskedEmail: maskEmail(client?.email ?? ''),
    });
    await silencePinPrompt('client', clientId);
  ...
  const enableBiometric = async () => {
    setBusy(true);
    const saved = await saveBiometricPin('client', clientId, first);
    if (saved) await markBiometricPin('client', clientId);
    setBusy(false);
    setStage('done');
  };
