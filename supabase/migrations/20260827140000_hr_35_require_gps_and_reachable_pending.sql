-- =============================================================================
-- Give require_gps its meaning, and make the pending path reachable.
--
-- hr_33 added require_gps as a setting and then never read it, and its
-- 'pending' approval branch could not be reached: every status that was not
-- 'inside' had already been refused above it. A setting that does nothing and a
-- branch that cannot run are both lies about how the system behaves.
--
-- The split is between a DEFINITE verdict and an INDEFINITE one. 'outside' is
-- definite and is always refused. 'unavailable' / 'inaccurate' / 'mock' mean we
-- do not know, and require_gps now decides whether not knowing is a refusal
-- (default, true) or a punch held for an administrator (false).
--
-- Behaviour with the shipped defaults is UNCHANGED: require_gps defaults to
-- true, which is exactly what hr_33 did unconditionally.
-- =============================================================================
...
BEGIN
   ...
   *
   * DEFINITE -- 'outside' means the position was read and it is not the office.
   * That is always a refusal: the person is demonstrably somewhere else and no
   * administrator needs to adjudicate it.
   *
   * INDEFINITE -- 'unavailable', 'inaccurate' and 'mock' mean we do not know
   * where they are. require_gps decides what happens to those. True (the
   * default) refuses, so nobody's hours rest on an unverified claim. False
   * lets the punch through as PENDING for an administrator, which is the right
   * setting while a fleet of older phones is still being sorted out -- someone
   * whose GPS chip is failing should not simply lose their attendance.
   */
  IF s.location_mode = 'enforce' AND NOT prof.location_exempt
     AND (s.require_gps OR v_loc_status = 'outside') THEN
  ...
  -- Reachable here in enforce mode only when the verdict was 'inside', when no
  -- geofence has been drawn yet, or when require_gps is off and the position
  -- could not be established -- that last case is the one that lands pending.
