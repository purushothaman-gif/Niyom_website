-- Show clients + partners only VERIFIED bonds. The list/detail RPCs previously
-- gated on active + priced; add verification_status='verified' so unverified
-- (needs_review / pending / enriching) bonds never reach a client or partner,
-- who — unlike staff — cannot judge data quality. Keeps the bm_public_analytics
-- price-leak strip.
...
BEGIN
  ...
  RETURN QUERY
    ...
    WHERE b.active_status='active' AND b.verification_status='verified' AND b.latest_price IS NOT NULL
...
BEGIN
  ...
  RETURN QUERY
    SELECT b.id, b.isin, b.bond_name, i.name, b.coupon_rate, b.coupon_type, b.coupon_frequency,
           ...
           b.min_investment, b.face_value, round(b.latest_price * (1 + v_m/100), 4), bm_public_analytics(b.analytics)
    FROM bm_bonds b LEFT JOIN bm_issuers i ON i.id = b.issuer_id
    WHERE b.id = p_id AND b.active_status='active' AND b.verification_status='verified' AND b.latest_price IS NOT NULL;
END; $$;
