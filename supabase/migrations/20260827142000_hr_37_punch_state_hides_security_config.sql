-- =============================================================================
-- Stop handing the anti-spoofing configuration to every caller of hr_punch_state.
--
-- hr_36 made hr_attendance_settings HR-readable, then this function went on
-- returning max_accuracy_metres and trusted_proxy_hops in its payload to every
-- employee -- closing the front door and leaving the window open. The accuracy
-- threshold is the more useful of the two to someone feeding the endpoint a
-- fabricated position: it is the exact number a fake fix must claim to beat.
--
-- detected_ip and forwarded_for STAY. Those describe the caller's own
-- connection, which they can read off any "what is my IP" page; they are not
-- configuration, and the attendance admin screen uses them to diagnose the
-- proxy chain.
--
-- The admin screen reads the hop count from hr_attendance_settings instead,
-- which it was already loading and is allowed to read.
-- =============================================================================
