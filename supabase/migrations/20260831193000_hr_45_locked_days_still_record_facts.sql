-- =============================================================================
-- A locked day stops lying about what happened.
--
-- Salary here is paid on the last WORKING day of the month, so payroll is
-- calculated and locked while that day is still being worked. Locking froze
-- every attendance row for the month, and hr_recompute_daily then refused to
-- touch a locked day at all -- so the punch-outs people made that evening were
-- recorded in hr_attendance_punches and never reached the day they belonged
-- to.
--
-- On 31 August 2026 that left five employees showing "worked 0 minutes, no
-- punch out" against real out-punches at 18:02-18:05, permanently, because the
-- nightly job skips locked days too. Every month would have added another such
-- day.
--
-- THE LOCK PROTECTS PAY, NOT FACTS. It exists so a settled payslip cannot be
-- contradicted later, and that is worth protecting absolutely. It was also
-- suppressing the punch times, worked minutes and lateness, which pay does not
-- depend on and which nobody has any reason to freeze.
--
-- So a locked day is now recomputed like any other, except that status,
-- payable_fraction and the leave / holiday / adjustment links keep the values
-- payroll settled on. What was paid cannot move; what was observed stays true.
--
-- Rebased on hr_38, the current definition -- not on hr_09 which first wrote
-- this function. (hr_41 was rebuilt from the wrong version and silently
-- reverted a fix; the rule is to start from the latest.)
-- =============================================================================
