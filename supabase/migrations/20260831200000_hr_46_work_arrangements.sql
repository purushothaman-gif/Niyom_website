-- =============================================================================
-- Record that somebody is working from home, instead of paying them by hand.
--
-- An employee on maternity leave who is working from home was being marked
-- ABSENT every day -- she has no office to punch in at -- and the only way to
-- pay her correctly was to waive five loss-of-pay days by hand at the end of
-- each month. That is a monthly manual correction standing in for a fact
-- nobody could record, and the month it gets forgotten she is underpaid.
--
-- A work arrangement is a date range during which an employee is at work
-- without punching: maternity working from home, a medical restriction, a
-- posting to a client site. Days inside it settle as ON DUTY and pay in full.
--
-- WHAT IT DOES NOT OVERRIDE. It is applied after holidays, weekly offs,
-- approved leave and admin corrections, so all of those still win -- taking
-- leave during a remote period is still leave. And it only rescues a day that
-- would otherwise be absence with no punches at all: if she does come to the
-- office and punch, the punches decide the day exactly as usual.
--
-- Open-ended by design (to_date NULL). A maternity arrangement rarely has a
-- firm end date when it starts, and an arrangement that silently expired would
-- reintroduce the very problem it fixes.
-- =============================================================================

CREATE TABLE IF NOT EXISTS public.hr_work_arrangements (
  id          uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  employee_id uuid NOT NULL REFERENCES public.nw_employees(id) ON DELETE CASCADE,

  kind        text NOT NULL DEFAULT 'remote'
                CHECK (kind IN ('remote', 'field', 'deputation', 'other')),
  from_date   date NOT NULL,
  to_date     date,                       -- NULL = still running
  -- Shown on the attendance register against every day it settles, so the
  -- reason a day was paid without a punch is visible where the day is.
...
CREATE INDEX IF NOT EXISTS hr_work_arrangements_lookup_idx
  ON public.hr_work_arrangements (employee_id, from_date DESC)
  WHERE status = 'active';

DROP TRIGGER IF EXISTS hr_work_arrangements_touch ON public.hr_work_arrangements;
CREATE TRIGGER hr_work_arrangements_touch BEFORE UPDATE ON public.hr_work_arrangements
  FOR EACH ROW EXECUTE FUNCTION public.hr_touch_updated_at();

COMMENT ON TABLE public.hr_work_arrangements IS
  'Periods during which an employee works without punching -- maternity working from home, field posting, deputation. Days inside settle as on_duty and pay in full, but never override leave, holidays, weekly offs, corrections, or an actual punch.';

ALTER TABLE public.hr_work_arrangements ENABLE ROW LEVEL SECURITY;

-- An employee may see their own arrangement -- it explains their own register.
CREATE POLICY hr_work_arrangements_read_self ON public.hr_work_arrangements
  FOR SELECT TO authenticated
  USING (employee_id = (SELECT nw_current_employee_id()) OR (SELECT hr_can_view('attendance')));

CREATE POLICY hr_work_arrangements_write ON public.hr_work_arrangements
