/*
  # Transfer Queue — a deal is eligible only when it is CONFIRMED (booked) and PAID

  Owner-confirmed rule for the Transfer Queue:
    1. Deal confirmation done  → nw_deal_confirmations.status = 'confirmed'
       (the deal has been booked; drafts never qualify).
    2. Fully paid              → settled within the existing ±₹50 tolerance.
    ...and it has not already been transferred.

  Why this migration exists: since payments can now be recorded against
  unconfirmed (draft) deals, a paid *draft* leaked into the queue because the
  eligibility view only gated on acceptance_status + the money tolerance and
  never checked the booking `status`. Add `status = 'confirmed'`.

  Two authoritative gates are updated so the rule holds end to end:
    - nw_deal_transfer_eligible  → what the queue lists.
    - nw_transfer_deal RPC       → the atomic transfer; add the same confirmed
      guard alongside its existing ±₹50 money re-check (transfers are
      irreversible, so both gates are enforced under the row lock too).

  Columns / signatures are unchanged — the front-end depends on them.
*/

-- ===========================================================================
-- 1. Eligibility view — add the CONFIRMED gate
-- ===========================================================================
...
WHERE d.status = 'confirmed'                              -- deal confirmation done (booked, not draft)
  ...
  AND ABS(s.outstanding_amount) <= 50                     -- fully paid within ±50 tolerance
  AND (t.id IS NULL OR t.transfer_stage IS DISTINCT FROM 'transferred');  -- not yet transferred

-- ===========================================================================
-- 2. Transfer RPC — enforce the CONFIRMED gate under the row lock too
--    (verbatim reproduction of 20260722120000, with one added guard in step (d))
-- ===========================================================================
