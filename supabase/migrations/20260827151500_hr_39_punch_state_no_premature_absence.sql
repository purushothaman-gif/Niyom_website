-- =============================================================================
-- The punch card stops answering "absent" when it simply has no row yet.
--
-- Companion to hr_38, which stopped hr_recompute_daily from calling an
-- unfinished day absence. hr_punch_state had the same assumption baked into a
-- COALESCE default: no daily row meant 'absent', which is a verdict on a day
-- that may not have started. It now mirrors hr_38 -- open until office_end.
-- =============================================================================
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    /*
     * No daily row yet -- the nightly job has not reached this employee, or
     * they were created today. Do not answer 'absent': that is a verdict on a
     * day that may not even have started. Mirror what hr_recompute_daily would
     * say, which is that the day is still open until office_end passes.
     */
    'status',           COALESCE(v_daily.status,
                          CASE WHEN (now() AT TIME ZONE 'Asia/Kolkata')
                                    < (v_date + s.office_end)
                               THEN 'upcoming' ELSE 'absent' END),
