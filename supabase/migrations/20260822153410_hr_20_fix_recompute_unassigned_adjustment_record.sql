-- =============================================================================
-- Fix: hr_recompute_daily() crashed with
--   "record \"adj\" is not assigned yet"
-- for any date outside an employee's employment window.
--
-- The `not_joined` and `exited` branches skip the ELSE block that runs
--   SELECT * INTO adj FROM hr_attendance_adjustments ...
-- but the INSERT at the bottom still reads `adj.id`. A PL/pgSQL `record` that
-- has never been assigned has an indeterminate row type, so touching a field on
-- it raises rather than returning NULL. (Assigning it from a zero-row SELECT is
-- fine -- the structure comes from the query. The failure needs the SELECT to
-- not run at all, which is why ordinary days never hit it.)
--
-- HOW BAD: hr_cron_nightly() calls hr_recompute_all_for_date() for EVERY active
-- employee. One employee with a future joining date -- an ordinary thing to
-- record when someone is hired -- would abort the nightly recomputation for the
-- entire company, silently, every night.
--
-- The fix drops the record variable for scalars, which cannot be
-- "unassigned": they are NULL until set, and every read below tolerates NULL.
-- =============================================================================
...
DECLARE
  ...
  -- Scalars, not a record: see the note above.
  v_adj_id          uuid;
